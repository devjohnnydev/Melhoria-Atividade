...
async function seedDatabase() {
  ...
  if (existingTeams.length === 0) {
    await storage.createTeam({
      name: "Squad Alpha",
      techLeadName: "João",
      productOwnerName: "Maria"
    });
    await storage.createTeam({
      name: "Bug Hunters",
      techLeadName: "Lucas",
      productOwnerName: "Pedro"
    });
    await storage.createTeam({
      name: "Code Ninjas",
      techLeadName: "Ana",
      productOwnerName: "Carla"
    });

    // Simulate some progress
    await storage.updateTeamPhase(1, 2); // Squad Alpha na fase 2
    await storage.updateTeamPhase(2, 4); // Bug Hunters na fase 4
  }
}
